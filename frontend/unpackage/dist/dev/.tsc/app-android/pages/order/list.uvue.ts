  ...
  methods: {
    ...
    handleCancelOrder(order : OrderVO) {
      uni.showModal({ title: '提示', content: '确定取消订单吗？', success: (res) => {
        if (res.confirm) {
          cancelOrder(order.id, '用户取消').then(() => { order.status = 4; order.statusDesc = '已取消'; uni.showToast({ title: '订单已取消', icon: 'success' }) }).catch((err) => { console.error('取消订单失败:', err, " at pages/order/list.uvue:231"); uni.showToast({ title: '取消失败', icon: 'none' }) })
        }
      }})
    },
    ...
    handleConfirmReceipt(order : OrderVO) {
      uni.showModal({ title: '提示', content: '确认已收到货吗？', success: (res) => {
        if (res.confirm) {
          confirmReceive(order.id).then(() => { order.status = 3; order.statusDesc = '已完成'; uni.showToast({ title: '确认收货成功', icon: 'success' }) }).catch((err) => { console.error('确认收货失败:', err, " at pages/order/list.uvue:243"); uni.showToast({ title: '操作失败', icon: 'none' }) })
        }
      }})
    },
    ...
    goToReview(order : OrderVO) {
      ...
      uni.navigateTo({
        url: `/pages/order/review?orderId=${order.id}&productId=${target.productId}&productName=${UTSAndroid.consoleDebugError(encodeURIComponent(target.productName), " at pages/order/list.uvue:279")}&productImage=${UTSAndroid.consoleDebugError(encodeURIComponent(target.productImage), " at pages/order/list.uvue:279")}`
      })
    },
...
const _component_BottomNav = resolveComponent("BottomNav")
  ...
  return createElementVNode("view", utsMapOf({ class: "order-page" }), [
    createElementVNode("view", utsMapOf({ class: "tab-header" }), [
      ...
      createElementVNode("view", utsMapOf({
        class: normalizeClass(["tab-item", utsMapOf({ active: _ctx.currentTab === 'pending' })]),
        onClick: () => {_ctx.switchTab('pending')}
      }), [
        createElementVNode("text", utsMapOf({
          class: normalizeClass(["tab-text", utsMapOf({ 'tab-text--active': _ctx.currentTab === 'pending' })])
        }), "待付款", 2 /* CLASS */),
            ...
            createElementVNode(Fragment, null, RenderHelpers.renderList(_ctx.orders, (order, __key, __index, _cached): any => {
              return createElementVNode("view", utsMapOf({
                ...
                createElementVNode("view", utsMapOf({ class: "card-header" }), [
                  createElementVNode("text", utsMapOf({ class: "order-number" }), "订单号：" + toDisplayString(order.orderNo), 1 /* TEXT */),
                  createElementVNode("text", utsMapOf({
                  ...
                  createElementVNode("view", utsMapOf({ class: "btn-group" }), [
                    order.status === 0
                      ? createElementVNode("view", utsMapOf({
                          ...
                          onClick: withModifiers(() => {_ctx.handleCancelOrder(order)}, ["stop"])
                        }), [
                          createElementVNode("text", utsMapOf({ class: "btn-text secondary-text" }), "取消订单")
                        ], 8 /* PROPS */, ["onClick"])
                      : createCommentVNode("v-if", true),
                    order.status === 0
                      ? createElementVNode("view", utsMapOf({
                          ...
                          onClick: withModifiers(() => {_ctx.handlePayOrder(order)}, ["stop"])
                        }), [
                          createElementVNode("text", utsMapOf({ class: "btn-text primary-text" }), "去支付")
                        ], 8 /* PROPS */, ["onClick"])
                      : createCommentVNode("v-if", true),
                    order.status === 2
                      ? createElementVNode("view", utsMapOf({
                          key: 2,
                          class: "action-btn primary",
                          onClick: withModifiers(() => {_ctx.handleConfirmReceipt(order)}, ["stop"])
                        }), [
                          createElementVNode("text", utsMapOf({ class: "btn-text primary-text" }), "确认收货")
                        ], 8 /* PROPS */, ["onClick"])
                      : createCommentVNode("v-if", true),
                    isTrue(order.status === 3 || order.status === 4 || order.status === 5)
                      ? createElementVNode("view", utsMapOf({
                          key: 3,
                          class: "action-btn secondary",
                          onClick: withModifiers(() => {_ctx.handleDeleteOrder(order)}, ["stop"])
                        }), [
                          createElementVNode("text", utsMapOf({ class: "btn-text secondary-text" }), "删除订单")
                        ], 8 /* PROPS */, ["onClick"])
                      : createCommentVNode("v-if", true),
                    isTrue(order.status === 3 && order.reviewed != true)
                      ? createElementVNode("view", utsMapOf({
                          key: 4,
                          class: "action-btn primary",
                          onClick: withModifiers(() => {_ctx.goToReview(order)}, ["stop"])
                        }), [
                          createElementVNode("text", utsMapOf({ class: "btn-text primary-text" }), "去评价")
                        ], 8 /* PROPS */, ["onClick"])
                      : createCommentVNode("v-if", true),
                    order.status === 3
                      ? createElementVNode("view", utsMapOf({
                          key: 5,
                          class: "action-btn primary",
                          onClick: withModifiers(() => {_ctx.buyAgain(order)}, ["stop"])
                        }), [
                          createElementVNode("text", utsMapOf({ class: "btn-text primary-text" }), "再次购买")
                        ], 8 /* PROPS */, ["onClick"])
                      : createCommentVNode("v-if", true)
                  ])
                ])
...
const GenPagesOrderListStyles = [utsMapOf([["order-page", padStyleMapOf(utsMapOf([["backgroundColor", "#f7f8fa"], ["display", "flex"], ["flexDirection", "column"], ["width", "100%"], ["height", "100%"]]))], ["tab-header", padStyleMapOf(utsMapOf([["backgroundColor", "#ffffff"], ["display", "flex"], ["flexDirection", "row"], ["paddingTop", CSS_VAR_STATUS_BAR_HEIGHT], ["zIndex", 100], ["boxShadow", "0 1px 2px rgba(0, 0, 0, 0.05)"]]))], ["tab-item", padStyleMapOf(utsMapOf([["flex", 1], ["height", 44], ["display", "flex"], ["flexDirection", "column"], ["alignItems", "center"], ["justifyContent", "center"], ["position", "relative"]]))], ["tab-text", padStyleMapOf(utsMapOf([["fontSize", 13], ["color", "#666666"]]))], ["tab-text--active", padStyleMapOf(utsMapOf([["color", "#0066CC"], ["fontWeight", "700"]]))], ["active-line", padStyleMapOf(utsMapOf([["position", "absolute"], ["bottom", 0], ["width", 24], ["height", 3], ["backgroundColor", "#0066CC"], ["borderTopLeftRadius", 2], ["borderTopRightRadius", 2], ["borderBottomRightRadius", 2], ["borderBottomLeftRadius", 2]]))], ["order-scroll", padStyleMapOf(utsMapOf([["flex", 1], ["width", "100%"], ["height", 0]]))], ["loading-state", padStyleMapOf(utsMapOf([["paddingTop", 60], ["paddingRight", 0], ["paddingBottom", 60], ["paddingLeft", 0], ["display", "flex"], ["alignItems", "center"], ["justifyContent", "center"]]))], ["loading-text", padStyleMapOf(utsMapOf([["fontSize", 14], ["color", "#999999"]]))], ["order-list", padStyleMapOf(utsMapOf([["paddingTop", 12], ["paddingRight", 16], ["paddingBottom", 12], ["paddingLeft", 16], ["display", "flex"], ["flexDirection", "column"]]))], ["order-card", padStyleMapOf(utsMapOf([["backgroundColor", "#ffffff"], ["borderTopLeftRadius", 12], ["borderTopRightRadius", 12], ["borderBottomRightRadius", 12], ["borderBottomLeftRadius", 12], ["paddingTop", 16], ["paddingRight", 16], ["paddingBottom", 16], ["paddingLeft", 16], ["marginBottom", 12], ["boxShadow", "0 2px 8px rgba(0, 0, 0, 0.02)"], ["display", "flex"], ["flexDirection", "column"]]))], ["card-header", padStyleMapOf(utsMapOf([["display", "flex"], ["flexDirection", "row"], ["justifyContent", "space-between"], ["alignItems", "center"], ["marginBottom", 16]]))], ["order-number", padStyleMapOf(utsMapOf([["fontSize", 13], ["color", "#666666"]]))], ["order-status", padStyleMapOf(utsMapOf([["fontSize", 13], ["fontWeight", "400"]]))], ["status-pending", padStyleMapOf(utsMapOf([["color", "#ff9800"]]))], ["status-unshipped", padStyleMapOf(utsMapOf([["color", "#2196f3"]]))], ["status-shipped", padStyleMapOf(utsMapOf([["color", "#0066CC"]]))], ["status-completed", padStyleMapOf(utsMapOf([["color", "#999999"]]))], ["status-cancelled", padStyleMapOf(utsMapOf([["color", "#cccccc"]]))], ["status-closed", padStyleMapOf(utsMapOf([["color", "#cccccc"]]))], ["goods-preview", padStyleMapOf(utsMapOf([["marginBottom", 16], ["display", "flex"], ["flexDirection", "column"]]))], ["single-goods", padStyleMapOf(utsMapOf([["display", "flex"], ["flexDirection", "row"]]))], ["goods-img", padStyleMapOf(utsMapOf([["width", 70], ["height", 70], ["borderTopLeftRadius", 8], ["borderTopRightRadius", 8], ["borderBottomRightRadius", 8], ["borderBottomLeftRadius", 8], ["backgroundColor", "#f5f5f5"], ["marginRight", 12]]))], ["goods-info", padStyleMapOf(utsMapOf([["flex", 1], ["display", "flex"], ["flexDirection", "column"], ["justifyContent", "flex-start"]]))], ["goods-name", padStyleMapOf(utsMapOf([["fontSize", 14], ["color", "#333333"], ["marginBottom", 8], ["lineHeight", 1.4]]))], ["goods-spec", padStyleMapOf(utsMapOf([["fontSize", 12], ["color", "#999999"], ["backgroundColor", "#f5f5f5"], ["paddingTop", 2], ["paddingRight", 6], ["paddingBottom", 2], ["paddingLeft", 6], ["borderTopLeftRadius", 4], ["borderTopRightRadius", 4], ["borderBottomRightRadius", 4], ["borderBottomLeftRadius", 4], ["alignSelf", "flex-start"]]))], ["goods-price-qty", padStyleMapOf(utsMapOf([["textAlign", "right"], ["display", "flex"], ["flexDirection", "column"], ["alignItems", "flex-end"], ["minWidth", 60]]))], ["price", padStyleMapOf(utsMapOf([["fontSize", 14], ["color", "#333333"], ["fontWeight", "700"], ["marginBottom", 4]]))], ["qty", padStyleMapOf(utsMapOf([["fontSize", 12], ["color", "#999999"]]))], ["multi-goods", padStyleMapOf(utsMapOf([["display", "flex"], ["flexDirection", "row"], ["alignItems", "center"]]))], ["goods-img-scroll", padStyleMapOf(utsMapOf([["flex", 1], ["whiteSpace", "nowrap"], ["marginRight", 12]]))], ["goods-img-list", padStyleMapOf(utsMapOf([["display", "flex"], ["flexDirection", "row"]]))], ["goods-img-small", padStyleMapOf(utsMapOf([["width", 60], ["height", 60], ["borderTopLeftRadius", 8], ["borderTopRightRadius", 8], ["borderBottomRightRadius", 8], ["borderBottomLeftRadius", 8], ["backgroundColor", "#f5f5f5"], ["marginRight", 8]]))], ["total-qty-info", padStyleMapOf(utsMapOf([["textAlign", "right"], ["minWidth", 80], ["display", "flex"], ["flexDirection", "column"], ["alignItems", "flex-end"]]))], ["card-footer", padStyleMapOf(utsMapOf([["display", "flex"], ["flexDirection", "row"], ["justifyContent", "flex-end"], ["alignItems", "center"], ["borderTopWidth", 1], ["borderTopStyle", "solid"], ["borderTopColor", "#f5f5f5"], ["paddingTop", 12]]))], ["total-info", padStyleMapOf(utsMapOf([["marginRight", "auto"], ["display", "flex"], ["flexDirection", "row"], ["alignItems", "center"]]))], ["label", padStyleMapOf(utsMapOf([["fontSize", 12], ["color", "#999999"], ["marginRight", 4]]))], ["amount", padStyleMapOf(utsMapOf([["fontSize", 16], ["fontWeight", "700"], ["color", "#333333"]]))], ["btn-group", padStyleMapOf(utsMapOf([["display", "flex"], ["flexDirection", "row"]]))], ["action-btn", padStyleMapOf(utsMapOf([["paddingTop", 0], ["paddingRight", 16], ["paddingBottom", 0], ["paddingLeft", 16], ["height", 32], ["borderTopLeftRadius", 16], ["borderTopRightRadius", 16], ["borderBottomRightRadius", 16], ["borderBottomLeftRadius", 16], ["display", "flex"], ["alignItems", "center"], ["justifyContent", "center"], ["marginTop", 0], ["marginRight", 0], ["marginBottom", 0], ["marginLeft", 8]]))], ["secondary", padStyleMapOf(utsMapOf([["backgroundColor", "#ffffff"], ["borderTopWidth", 1], ["borderRightWidth", 1], ["borderBottomWidth", 1], ["borderLeftWidth", 1], ["borderTopStyle", "solid"], ["borderRightStyle", "solid"], ["borderBottomStyle", "solid"], ["borderLeftStyle", "solid"], ["borderTopColor", "#dddddd"], ["borderRightColor", "#dddddd"], ["borderBottomColor", "#dddddd"], ["borderLeftColor", "#dddddd"]]))], ["primary", padStyleMapOf(utsMapOf([["backgroundColor", "#ffffff"], ["borderTopWidth", 1], ["borderRightWidth", 1], ["borderBottomWidth", 1], ["borderLeftWidth", 1], ["borderTopStyle", "solid"], ["borderRightStyle", "solid"], ["borderBottomStyle", "solid"], ["borderLeftStyle", "solid"], ["borderTopColor", "#0066CC"], ["borderRightColor", "#0066CC"], ["borderBottomColor", "#0066CC"], ["borderLeftColor", "#0066CC"]]))], ["btn-text", padStyleMapOf(utsMapOf([["fontSize", 13]]))], ["secondary-text", padStyleMapOf(utsMapOf([["color", "#666666"]]))], ["primary-text", padStyleMapOf(utsMapOf([["color", "#0066CC"]]))], ["empty-state", padStyleMapOf(utsMapOf([["paddingTop", 60], ["paddingRight", 0], ["paddingBottom", 60], ["paddingLeft", 0], ["display", "flex"], ["flexDirection", "column"], ["alignItems", "center"], ["justifyContent", "center"]]))], ["empty-icon", padStyleMapOf(utsMapOf([["fontSize", 48], ["marginBottom", 16], ["color", "#cccccc"]]))], ["empty-text", padStyleMapOf(utsMapOf([["fontSize", 14], ["color", "#999999"]]))], ["load-more", padStyleMapOf(utsMapOf([["paddingTop", 20], ["paddingRight", 0], ["paddingBottom", 20], ["paddingLeft", 0], ["display", "flex"], ["alignItems", "center"], ["justifyContent", "center"]]))], ["load-more-text", padStyleMapOf(utsMapOf([["fontSize", 12], ["color", "#999999"]]))]])]
