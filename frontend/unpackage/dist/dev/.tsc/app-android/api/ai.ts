...
export function chatWithAiStream(
	...
	__f__('log','at api/ai.ts:39',`${logTag} start`, {
		question: question,
		sessionId: sessionId
	})
	...
	__f__('log','at api/ai.ts:51',`${logTag} platform`, 'APP SSE via uni.request')
	...
	__f__('log','at api/ai.ts:58',`${logTag} request`, {
		url: url,
		headers: {
	...
	const reqOpts: RequestOptions<string> = {
		...
		success: function(res: RequestSuccess<string>) {
			if (res != null) {
				__f__('log','at api/ai.ts:82',`${logTag} response`, res)
